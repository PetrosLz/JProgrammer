...
| W-034 | Demo Cafe exact acceptance | Reset local data and load current Demo Cafe. | Generate week `2026-05-18` to `2026-05-24`. | Headless baseline: 40 requested slots, 28 unique assigned, 12 unfilled, 0 hard violations, status `Understaffed`, coverage proof false, coverage ceiling 28, Saturday unfilled slots 12, and heuristic fallback in Node/headless verification. Real Electron CP-SAT-first manual acceptance: engine/status must match the actual runtime. Use CP-SAT when the packaged/local solver is available and valid; heuristic fallback only when the solver is unavailable, errors, reports fallback status, or TypeScript validation fails. Do not claim heuristic proof. UI/database/PDF must agree. |  |  |  |  |
